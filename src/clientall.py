...
input = torch.FloatTensor(features[0:400])
label = torch.FloatTensor(labels[0:400])
...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        self.linear = torch.nn.Linear(18, 18)  # One in and one out
        self.sm = torch.nn.Sigmoid()
        self.linear1 = torch.nn.Linear(18, 1)
...
for i in range(400, 437):
    ...
    pred_res = round(model.forward(hour_var).data[0][0].item() * 4)
    print(type(pred_res))
